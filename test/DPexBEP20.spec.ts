...
describe('DPexBEP20', () => {
  ...
  let token: IBEP20
  beforeEach(async () => {
    token = await deployContract(wallet, ERC20, [TOTAL_SUPPLY]) as IBEP20
  })

  it('name, symbol, decimals, totalSupply, balanceOf, DOMAIN_SEPARATOR, PERMIT_TYPEHASH', async () => {
    const name = await token.name()
    expect(name).to.eq('Decentralized Passive Exchange Pair')
    expect(await token.symbol()).to.eq('DPEX-LP')
